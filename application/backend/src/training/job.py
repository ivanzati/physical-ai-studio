    ...
    *without* writing a checkpoint or exporting, since a partially trained run
    ...
    with _hf_token_env(spec.run_options.hf_token):
        ...
        datamodule = LeRobotDataModule(
            ...
            # Training runs against the bundled PyAV/FFmpeg stack, while the
            # default torchcodec path has failed on real datasets.
            # LeRobot's PyAV path is not worker-safe here, so training keeps
